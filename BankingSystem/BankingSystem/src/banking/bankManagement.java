...
public class bankManagement { // this class provides all
							// bank method
	...
	public static boolean createAccount(String name,int passCode) // create account function
	{
		try {
			...
			sql = "insert into bank.customer(cname,balance,pass_code) values('"	+ name + "',1000," + passCode + ")";
	...
	public static boolean loginAccount(String name, int passCode) // login method
	{
		try {
			...
			// query
			sql = "select * from bank.customer where cname='"+ name + "' and pass_code=" + passCode;
	...
	public static void getBalance(int acNo) // fetch balance method
	{
		try {
			// query
			sql = "select * from bank.customer where ac_no="+ acNo;
			PreparedStatement st= con.prepareStatement(sql);
	...
	public static boolean transferMoney(int sender_ac,int reveiver_ac,int amount) throws SQLException // transfer money method
	{
		...
		try
		{
			...
			sql = "select * from bank.customer where ac_no="	+ sender_ac;
			...
			sql = "update bank.customer set balance=balance-"+ amount + " where ac_no=" + sender_ac;
			if (st.executeUpdate(sql) == 1) 
			{
			...
			// credit
			sql = "update bank.customer set balance=balance+"+ amount + " where ac_no=" + reveiver_ac;
